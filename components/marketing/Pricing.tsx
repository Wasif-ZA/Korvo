...
export function Pricing() {
  ...
  return (
    <section
      id="pricing"
      className="py-24 relative overflow-hidden bg-white border-b border-border"
    >
          ...
          {TIERS.map((tier, i) => (
            <div
              ...
              className={cn(
                ...
                tier.popular
                  ? "border-accent ring-1 ring-accent/20 scale-105 z-20 shadow-xl"
                  : "border-border",
              )}
            ...
            >
              ...
              <h3 className="text-[12px] font-mono font-bold text-text-muted uppercase tracking-[0.3em] mb-6">
                {tier.name}
              </h3>
              <div className="flex items-baseline gap-1 mb-12">
                <span className="text-[48px] font-serif font-bold text-text-primary">
                  {tier.price}
                </span>
                <span className="text-text-light text-[12px] font-mono">
                  /mo
                </span>
              </div>
              ...
              <ul className="space-y-6 mb-16 flex-1">
                {tier.features.map((feature) => (
                  <li
                    key={feature}
                    className="flex items-start gap-4 text-[13px] font-mono text-text-secondary group-hover:text-text-primary transition-colors"
                  >
