...
let mockUserResult: { user: { id: string; email: string } | null } = {
  user: null,
};
...
describe("proxy auth guard behavior", () => {
  ...
  describe("protected routes (/settings, /dashboard) — unauthenticated", () => {
    it("redirects to /login when user is null and path is /settings", async () => {
      mockUserResult = { user: null };
    ...
    it("redirects to /login when user is null and path is /dashboard", async () => {
      mockUserResult = { user: null };
      const { proxy } = await import("@/proxy");
    ...
    it("redirects to /login for nested protected paths like /settings/profile", async () => {
      mockUserResult = { user: null };
      const { proxy } = await import("@/proxy");
